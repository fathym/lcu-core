...
{
    public class LCUOpenIdConnectHandler : RemoteAuthenticationHandler<OpenIdConnectOptions>, IAuthenticationSignOutHandler
    {
        ...
        public LCUOpenIdConnectHandler(IHttpContextAccessor httpContextAccessor, IOptionsMonitor<OpenIdConnectOptions> options, ILoggerFactory logger, HtmlEncoder htmlEncoder, UrlEncoder encoder, ISystemClock clock)
        ...
        {
            return _httpContext.ResolveContext<EnterpriseContext>(EnterpriseContext.Lookup).ADB2CApplicationID ?? Options.ClientId;
        }
        ...
        protected override async Task<HandleRequestResult> HandleRemoteAuthenticateAsync()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(authorizationResponse.Error))
                {
                    ...
                    {
                        return HandleRequestResult.Fail("Authorization denied.", properties);
                ...
                validationParameters.ValidAudiences = validationParameters.ValidAudiences.AddItem(clientId);
                ...
                else
                {
                    ...
                    {
                        action.Run(JsonDocument.Parse("{}").RootElement, identity, ClaimsIssuer);
                    }
                }
        ...
        private async Task<AuthorizationCodeReceivedContext> RunAuthorizationCodeReceivedEventAsync(OpenIdConnectMessage authorizationResponse, ClaimsPrincipal user, AuthenticationProperties properties, JwtSecurityToken jwt)
        {
            ...
            context.Backchannel.BaseAddress = Backchannel.BaseAddress;

            Backchannel.DefaultRequestHeaders.Each(header =>
            {
                context.Backchannel.DefaultRequestHeaders.TryAddWithoutValidation(header.Key, header.Value);
            });

            context.Backchannel.Timeout = Backchannel.Timeout;

            context.Backchannel.MaxResponseContentBufferSize = Backchannel.MaxResponseContentBufferSize;
